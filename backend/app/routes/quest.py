...
def create_quest():
    ...
    return (
        jsonify({"message": "Quest created successfully", "quest_id": new_quest.id}),
        201,
    )
...
def get_user_quests():
    ...
    quests = (
        Quest.query.filter_by(author_id=user_id)
...
def upload_quest_image(quest_id):
    ...
    if int(quest.author_id) != int(user_id):
        return (
            jsonify(
                {"message": "You are not authorized to upload an image for this quest"}
            ),
            403,
        )
    ...
    if file and allowed_file(file.filename):
        ...
        return (
            jsonify(
                {
                    "message": "Image of quest updated successfully",

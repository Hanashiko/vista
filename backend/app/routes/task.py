...
def upload_task_image(quest_id, task_id):
    ...
    if int(quest.author_id) != int(user_id):
        return (
            jsonify(
                {"message": "You are not authorized to upload an image for this task"}
            ),
            403,
        )
    ...
    if file and allowed_file_image(file.filename):
        ...
        db.session.commit()
        logger.info(
            f"Image updated for task: {task.id} from quest: {quest.id} - {quest.title}"
        )
        return (
            jsonify(
                {
                    "message": "Image of task updated successfully",
                    "image_url": f"{request.host_url}v1/uploads/{filename}",
                }
            ),
            200,
        )
...
def uploads_task_video(quest_id, task_id):
    ...
    if int(quest.author_id) != int(user_id):
        return (
            jsonify(
                {"message": "You are not authorized to upload an video for this task"}
            ),
            403,
        )
    ...
    if file and allowed_file_video(file.filename):
        ...
        db.session.commit()
        logger.info(
            f"Video updated for task: {task.id} from quest: {quest.id} - {quest.title}"
        )
        return jsonify(
            {
                "message": "Video of task updated successfully",
                "image_url": f"{request.host_url}v1/uploads/{filename}",
            }
        )
    return jsonify({"message": "Invalid file type"}), 400
...
def edit_task(quest_id, task_id):
    ...
    if "options" in data:
        ...
        for option in data["options"]:
            new_option = TaskOption(
                text=option["text"], is_correct=option["is_correct"], task_id=task.id
            )
            db.session.add(new_option)
        db.session.commit()

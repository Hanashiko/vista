...
def profile():
    ...
    user_data = {
        ...
        "avatar": avatar_url,
        "quests_created": [
            {"id": quest.id, "title": quest.title} for quest in user.quests_created
        ],
        "quests_taken": [
            {"id": user_quest.quest.id, "title": user_quest.quest.title}
            for user_quest in user.quests_taken
        ],
    }
            ...
            jsonify(
                {
                    ...
                    "avatar_url": f"{request.host_url}v1/uploads/{filename}",
                }
            ),
            200,
        )

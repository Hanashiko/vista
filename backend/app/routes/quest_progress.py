...
def start_quest(quest_id):
    ...
    existing_progress = UserQuest.query.filter_by(user_id=user_id, quest_id=quest_id).first()
...
def answer_task(quest_id, task_id):
    ...
    progress = UserQuest.query.filter_by(user_id=user_id, quest_id=quest_id).first_or_404()
    ...
    if task.question_type == "multiple_choice":
        ...
        if all(option.text in user_answer for option in correct_options) and len(user_answer) == len(correct_options):
